...
static int cuda_chrdev_open(struct inode *inode, struct file *filp)
{
	...
	if (!cudev) {
		printk("virtio-cuda: Could not find device with %u minor", 
		      iminor(inode));
		ret = -ENODEV;
		goto fail;
	}
                                ...
                                unsigned long arg)
{
	...
	case CUMODULEGETGLOBAL:
		...
		if (copy_from_user(name, (void *)user_p.name, 100*sizeof(char))) {
                        ret = -EACCES;
                        goto out;
                }
		sg_init_one(&output_sg, &user_p, sizeof(user_p));
	...
	//allocate and initialize one scatterlist for each argument
	case CULAUNCHKERNEL:
	...
	if (cmd != CUMEMCPYHTOD && cmd != CUMEMCPYDTOH && cmd != CULAUNCHKERNEL && cmd != CUMEMCPYDTOHASYNC && cmd != CUMEMCPYHTODASYNC)
	...
	if (user_p.total_alloc >= 196612 && 
		(cmd == CUMEMCPYHTOD || cmd == CUMEMCPYDTOH || cmd == CUCTXSYNCHRONIZE 
		|| cmd == CUMEMCPYDTOHASYNC || cmd == CUMEMCPYHTODASYNC)) {
...
int cuda_chrdev_init(void)
{
	...
	ret = cdev_add(&cuda_chrdev_cdev, dev_no, 1);
	if (ret < 0) {
		printk ("virtio-cuda: failed to add character device");
		goto out;
	}
